...
This file contains the methods for explanations generation by using slightly adapted
Polleti/Cozman model (composed_relations are introduced).
"""
import train_model as tm
import TransE_model_allow_all_tails as tem
import numpy as np
import pandas as pd
...
def predict_head_samples_tails(head, features_names, head_samples,dataset,relation,model):
    ...
    by using TransE model, which is not restricted to the ground truth dataset.
    """
    head_relations_tails, relations = tem.predict_best_possible_tails_for_relations(head, features_names,dataset,model)
    ...
    for rel, tails in head_relations_tails:
        if (':' in rel): #if this is composed relation
            ...
            for i in range(len(various_samples)):
                 tails = tem.predict_best_possible_tails_TransE(various_samples[i], rels[1],dataset,model)
...
def generate_explanations(head, features_names, relation, tail,head_samples,dataset,model):
    ...
    #gs = GridSearchCV(LogisticRegression(), params, cv=8, n_jobs=12)
    #gs.fit(X, y)
